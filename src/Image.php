...
class Image
{
    ...
    /**
     * Função de processamento de dados de GPS de EXIF de imagem
     * @return null
     */
    public function getGPSExifInfo()
    {
        $lat = $this->exif['GPS']['GPSLatitude'];
        $log = $this->exif['GPS']['GPSLongitude'];
        if (empty($lat) or empty($log)) {
            return null;
        }

        // latitude values //
        list($num_degrees_lat, $dec_degrees_lat) = explode('/', $lat[0]);
        $lat_degrees = $num_degrees_lat / $dec_degrees_lat;
        list($num_minutes_lat, $dec_minutes_lat) = explode('/', $lat[1]);
        $lat_minutes = $num_minutes_lat / $dec_minutes_lat;
        list($num_seconds_lat, $dec_seconds_lat) = explode('/', $lat[2]);
        $lat_seconds = $num_seconds_lat / $dec_seconds_lat;

        $lat_hemi = $this->exif['GPS']['GPSLatitudeRef'];

        // longitude values //
        list($num_degrees_log, $dec_degrees_log) = explode('/', $log[0]);
        $log_degrees = $num_degrees_log / $dec_degrees_log;
        list($num_minutes_log, $dec_minutes_log) = explode('/', $log[1]);
        $log_minutes = $num_minutes_log / $dec_minutes_log;
        list($num_seconds_log, $dec_seconds_log) = explode('/', $log[2]);
        $log_seconds = $num_seconds_log / $dec_seconds_log;

        $log_hemi = $this->exif['GPS']['GPSLongitudeRef'];

        // latitude formatter with hemisfere
        $dLat = $lat_degrees + $lat_minutes / 60 + $lat_seconds / 3600;
        $lat_decimal = ($lat_hemi == 'S' || $lat_hemi == 'W') ? $dLat*=-1 : $dLat;

        // longitude formatter with hemisfere
        $dLog = $log_degrees + $log_minutes / 60 + $log_seconds / 3600;
        $log_decimal = ($log_hemi == 'S' || $log_hemi == 'W') ? $dLog*=-1 : $dLog;

        return array('lt' => $lat_decimal, 'lg' => $log_decimal);
    }
    ...
    /**
     * Return EXIF data witch image
     *
     * @return Object/Array
     */
    protected function exifImageData()
    {
        return $this->exif = exif_read_data($this->fileName, null, true, false);
    }
