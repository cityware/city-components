...
class Filesystem
{
     ...
     *
     * @throws \RuntimeException When originFile doesn't exist
     * @throws \RuntimeException When copy fails
     */
    public function copy($originFile, $targetFile, $override = false)
    {
        if (stream_is_local($originFile) && !is_file($originFile)) {
            throw new \RuntimeException(sprintf('Failed to copy "%s" because file does not exist.', $originFile), 0, null);
        }
        ...
        if ($doCopy) {
            ...
            if (!is_file($targetFile)) {
                throw new \RuntimeException(sprintf('Failed to copy "%s" to "%s".', $originFile, $targetFile), 0, null);
            }
        }
    }
    ...
    public function mkdir($dirs, $mode = 0777)
    {
        foreach ($this->toIterator($dirs) as $dir) {
            ...
            if (true !== @mkdir($dir, $mode, true)) {
                throw new \RuntimeException(sprintf('Failed to create "%s".', $dir), 0, null);
            }
        }
    }
    ...
    public function touch($files, $time = null, $atime = null)
    {
        foreach ($this->toIterator($files) as $file) {
            ...
            if (true !== $touch) {
                throw new \RuntimeException(sprintf('Failed to touch "%s".', $file), 0, null);
            }
        }
    }
    ...
    public function remove($files)
    {
        ...
        foreach ($files as $file) {
            ...
            if (is_dir($file) && !is_link($file)) {
                ...
                if (true !== @rmdir($file)) {
                    throw new \RuntimeException(sprintf('Failed to remove directory "%s".', $file), 0, null);
                }
            } else {
                // https://bugs.php.net/bug.php?id=52176
                if (defined('PHP_WINDOWS_VERSION_MAJOR') && is_dir($file)) {
                    if (true !== @rmdir($file)) {
                        throw new \RuntimeException(sprintf('Failed to remove file "%s".', $file), 0, null);
                    }
                } else {
                    if (true !== @unlink($file)) {
                        throw new \RuntimeException(sprintf('Failed to remove file "%s".', $file), 0, null);
    ...
    public function chmod($files, $mode, $umask = 0000, $recursive = false)
    {
        foreach ($this->toIterator($files) as $file) {
            ...
            if (true !== @chmod($file, $mode & ~$umask)) {
                throw new \RuntimeException(sprintf('Failed to chmod file "%s".', $file), 0, null);
            }
        }
    }
        ...
        foreach ($this->toIterator($files) as $file) {
            ...
            if (is_link($file) && function_exists('lchown')) {
                if (true !== @lchown($file, $user)) {
                    throw new \RuntimeException(sprintf('Failed to chown file "%s".', $file), 0, null);
                }
            } else {
        ...
        foreach ($this->toIterator($files) as $file) {
            ...
            if (is_link($file) && function_exists('lchgrp')) {
                if (true !== @lchgrp($file, $group)) {
                    throw new \RuntimeException(sprintf('Failed to chgrp file "%s".', $file), 0, null);
                }
            } else {
    ...
    public function rename($origin, $target, $overwrite = false)
    {
        ...
        if (!$overwrite && is_readable($target)) {
            throw new \RuntimeException(sprintf('Cannot rename because the target "%s" already exists.', $target), 0, null);
        }

        if (true !== @rename($origin, $target)) {
            throw new \RuntimeException(sprintf('Cannot rename "%s" to "%s".', $origin, $target), 0, null);
        }
    }
    ...
    public function symlink($originDir, $targetDir, $copyOnWindows = false)
    {
        ...
        if (!$ok) {
            if (true !== @symlink($originDir, $targetDir)) {
                ...
                throw new \RuntimeException(sprintf('Failed to create symbolic link from "%s" to "%s".', $originDir, $targetDir), 0, null);
            }
        }
    }
        ...
        foreach ($iterator as $file) {
            ...
            if ($copyOnWindows) {
                if (is_link($file) || is_file($file)) {
                    ...
                    throw new \RuntimeException(sprintf('Unable to guess "%s" file type.', $file), 0, null);
                }
            } else {
                if (is_link($file)) {
    ...
    public function dumpFile($filename, $content, $mode = 0666)
    {
        ...
        if (!is_dir($dir)) {
            ...
            throw new \RuntimeException(sprintf('Unable to write to the "%s" directory.', $dir), 0, null);
        }

        $tmpFile = tempnam($dir, basename($filename));

        if (false === @file_put_contents($tmpFile, $content)) {
            throw new \RuntimeException(sprintf('Failed to write file "%s".', $filename), 0, null);
        }
